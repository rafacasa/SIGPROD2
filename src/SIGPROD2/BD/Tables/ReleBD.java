package SIGPROD2.BD.Tables;

/**
 * Classe responsável por manter os nomes das colunas e da tabela de Relés.
 *
 * @author Rafael Casa
 * @version 06/04/2016
 */
public class ReleBD {

    public static final String CODIGO = "codigo";
    public static final String FABRICANTE = "fabricante";
    public static final String MODELO = "modelo";
    public static final String IS_DIGITAL = "isDigital";
    public static final String FATOR_INICIO_INVERSO_FASE = "fatorInicioInvFase";
    public static final String FATOR_INICIO_INVERSO_NEUTRO = "fatorInicioInvNeutro";
    public static final String FATOR_INICIO_DEFINIDO_FASE = "fatorInicioDefFase";
    public static final String FATOR_INICIO_DEFINIDO_NEUTRO = "fatorInicioDefNeutro";
    public static final String EXISTE_CURVA_INVERSA_FASE = "existeInversaFase";
    public static final String EXISTE_CURVA_INVERSA_NEUTRO = "existeInversaNeutro";
    public static final String EXISTE_CURVA_DEFINIDA_FASE = "existeDefinidaFase";
    public static final String EXISTE_CURVA_DEFINIDA_NEUTRO = "existeDefinidaNeutro";
    public static final String TABELA = "Rele";
}
